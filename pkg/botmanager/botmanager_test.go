...
func TestNewBotManager(t *testing.T) {
	tStart := time.Now()
	c := config.New()
	b, err := New(c.RobotsSourceURL, c.CacheUpdateInterval)
	if err != nil {
...
func TestBotManagerBadURL(t *testing.T) {
	...
	c.RobotsSourceURL = "%%"
	_, err := New(c.RobotsSourceURL, c.CacheUpdateInterval)
	if err == nil {
...
func TestGetBotIndex(t *testing.T) {
	...
	c := config.New()
	b, _ := New(c.RobotsSourceURL, c.CacheUpdateInterval)
	botI, err := b.GetBotIndex(log)
	if err != nil {
